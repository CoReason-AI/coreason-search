...
from coreason_search.schemas import AppConfig, EmbeddingConfig, ScoutConfig
...
class TestEngineConfig:
    ...
    def test_engine_initialization_with_config_object(self) -> None:
        """Test initializing engine with AppConfig object."""
        config = AppConfig(database_uri="/tmp/test_engine_db", scout=ScoutConfig(threshold=0.9))
        ...
        # Check Scout config
        # We need to access the scout instance's config.
        # MockScout stores it in self.config
    ...
    def test_engine_component_propagation(self) -> None:
        ...
        config = AppConfig(database_uri="/tmp/prop_db", embedding=EmbeddingConfig(model_name="prop-model"))
        engine = SearchEngine(config)
